...
/*   Updated: 2019/04/18 18:57:51 by aholster      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_valiflag(unsigned char *seg, t_flag *flags)
{
	...
	{
		if (seg[0] >= 64 &&\
			((1LLU << (seg[0] - 64)) & flags.doubleflags[1] == 1))
			return (1);
		else if ((1LLU << seg[0]) & flags.doubleflags[0] == 1))
			return (1);
	}	
	else if (seg[0] >= 64 &&\
			(1LLU << (seg[0] - 64)) & flags.standflags[1] == 1)
		return (1);
	else if (((1LLU << seg[0]) & flags.standflags[0]) == 1)
		return (1);
	return (-1);
}

static void	ft_flagreset(t_list *flags)
{
	flags.actiflags[0] = 0;
	flags.actiflags[1] = 0;
	flags.precision = 0;
