...
/*   Updated: 2019/06/07 19:25:37 by jesmith       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
							...
							t_print *clipb)
{
	...
	else if (clipb->flags->padding > clipb->flags->precision && clipb->flags->padding > nb_len)
	{
		if (pad_spaces(diff, clipb) == -1)
			return (-1);
	}
	else if (clipb->flags->padding > nb_len)// && flagverif('.', clipb->flags) == -1) //&& clipb->flags->padding > clipb->flags->precision )
	{
...
int						ft_decimal(va_list ap, t_print *clipb)
{
	...
	flagverif('.', clipb->flags) == 1)
		return (1);
	if (neg == -1 && clipb->flags->padding != 0)
